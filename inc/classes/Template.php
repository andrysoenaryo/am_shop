...
class Template {
    ...
    private function build_nav_array($nav_array) {
        ...
        foreach ($nav_array as $node) {
			...
			if(($node['url']=='#') || ($node['url']=='')){ $link_url = '#'; $target = 'target=""';}
			else{ $link_url = '#'; $target = "onclick=click_menu('".$node['url']."','".str_replace(' ','_',$link_name)."');"; }
            ...
            if ($link_type == 'heading') {
                ...
                $this->nav_html .= "<a$link_prop href=\"$link_url\" ".$target.">$link_icon$link_name</a>\n";
	...
	# Add Session Menu Privilege
	
	public function Privilege_menu($print = true)
	{
		$this->GetPrivilege_menu($this->main_nav);

        return  $_SESSION;

	}
	
	private function GetPrivilege_menu($sess_array)
	{
		# Privilege Action Button Menu
		foreach ($sess_array as $field) {
			$_SESSION['menu'][$field['id']]['name'] = $field['name'];
			$_SESSION['menu'][$field['id']]['ubah'] = $field['ubah'];
			$_SESSION['menu'][$field['id']]['hapus'] = $field['hapus'];
			$_SESSION['menu'][$field['id']]['tambah'] = $field['tambah'];
			$_SESSION['menu'][$field['id']]['tampil'] = $field['tampil'];
			
			if(isset($field['sub'])){$this->GetPrivilege_menu($field['sub']);}
		}
