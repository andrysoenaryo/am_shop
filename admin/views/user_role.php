...
<!--<link rel="stylesheet" href="<?php echo $one->assets_folder; ?>/js/plugins/bootstrap-table/src/bootstrap-table.css">-->
    ...
    <div class="block-content tab-content">
        <div class="tab-pane active" id="tab-list">
            <div class="table-responsive"><table id="table_user_role" ></table></div>
        </div>
                ...
                <div class="form-group">
                    <div class="col-sm-6">
                        <div class="form-material  form-material-info ">
                            <select class="form-control" id="username" name="username" size="1">
                            </select>
...
$(function() {
	...
	$('#table_user_role').bootstrapTable({
		url				: 'admin/model/mod_user_role.php',
		method			: 'get',
		dataType		: 'json',
		queryParams		: {
							grid_table	: 1,
...
function action(act,tab,id_role,username)
{
	var URL = 'admin/model/mod_user_role.php';
	var act = act;
	if(act=='back')
	{
		backTabs();
	...
	else if(act=='simpan')
	{
		$.ajax({
			...
			success:function(data){				
				...
				backTabs();
				$('#table_user_role').bootstrapTable('refresh');
				clearAll();
			}
		});	
	}
...
$(function(){
  var items="";
  $.getJSON("admin/model/mod_user_role.php?select=username",function(data){
	  items+="<option value=''></option>";
	$.each(data,function(index,item) 
	{
	  items+="<option value='"+item.username+"'>"+item.nama_lengkap+"</option>";
	});
	$("#username").html(items); 
  });
});
...
$(function(){
  var items="";
  $.getJSON("admin/model/mod_user_role.php?select=role",function(data){
	  items+="<option value=''></option>";
	$.each(data,function(index,item) 
	{
