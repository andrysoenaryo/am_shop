<?php require '../../inc/config.php'; ?>
    ...
    <div class="block-content tab-content">
        <div class="tab-pane active" id="tab-list">
            <div class="table-responsive"><table id="table_icon" ></table></div>
        </div>
        <div class="tab-pane" id="tab-form">
            <div id="form" class="form-horizontal push-10-t">
                ...
                <input class="form-control" type="hidden" id="id_icon" name="id_icon" >
                <input class="form-control" type="hidden" id="status" name="status" >
            	<div class="form-group">
                    <div class="col-sm-4">
                        <div class="form-material form-material-info ">
                            <input class="form-control" type="text" id="nm_icon" name="nm_icon" >
                            <label for="nm_icon">Nama Icon</label>
                        </div>
                    </div>
                </div>
                <div class="form-group">
                    <div class="col-sm-4">
                        <div class="form-material form-material-info ">
                            <input class="form-control" type="text" id="class_icon" name="class_icon" >
                            <label for="class_icon">Class</label>
                        </div>
                    </div>
                </div>
                <div class="form-group">
...
$(function() {
	...
	$('#table_icon').bootstrapTable({
		url				: 'admin/model/mod_icon.php',
		method			: 'get',
		dataType		: 'json',
		queryParams		: {
							grid_table	: 1,
		...
		columns:[[
				...
				{field:'id_icon',title:'ID Icon'},
				{field:'nm_icon',title:'Nama Icon'},
				{field:'class_icon',title:'Class Icon'},
...
{
	var URL = 'admin/model/mod_icon.php';
	var act = act;
	if(act=='back')
	{
		backTabs();
	...
	else if(act=='edit')
	{
		$.ajax({
			...
			success:function(data){				
				nextTabs();
				$("#nm_icon").val(data.nama_icon);
				$("#id_icon").val(data.id_icon);
				$("#class_icon").val(data.class);
				$("#status").val(act);
	...
	else if(act=='simpan')
	{
		$.ajax({
			...
			success:function(data){				
				...
				backTabs();
				$('#table_icon').bootstrapTable('refresh');
				clearAll();
			}
		});	
	}
