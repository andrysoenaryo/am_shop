<?php require '../../inc/config.php'; 
	...
	if(isset($_GET['grid_table']))
	{
		...
		$sql = "select b.title as parent, a.* from menu a 
				left join menu b on a.parent_id = b.id 
				order by parent_id,level, menu_order asc";
		$conn->query($sql);
		$row = $conn->resultset();
		...
		foreach($row as $key => $value)
		{
			...
			$data[$key]['class_images'] = $value['class_images'];
			$data[$key]['active'] 		= $value['status']==0 ? "<span class=\"label label-success\">Active</span>" : "<span class=\"label label-danger\">Not Active</span>";
			$data[$key]['action'] 		= "<button ".$disable_ubah." class = \"btn btn-sm btn-success\" onClick=\"action('edit','tab-form','".$value['id']."');\"> Edit </button> 
										   <button ".$disable_hapus." class = \"btn btn-sm btn-danger\" onClick=\"action('delete','delete','".$value['id']."');\"> Delete </button>";
		}
